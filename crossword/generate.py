...
class CrosswordCreator():
    ...
    def revise(self, X, Y):
        ...
        """
        revised = False
        d1 = copy.deepcopy(self.domains[X])
        d2 = copy.deepcopy(self.domains[Y])
        to_remove = {x: len(d2) for x in d1}
    ...
    def consistent(self, assignment):
        ...
        for i in assignment:
            ...
            for j in assignment:
                if i != j:
                    if (i,j) in self.crossword.overlaps and self.crossword.overlaps[(i,j)] != None:
    ...
    def order_domain_values(self, var, assignment):
        ...
        l = {x: count for x in self.domains[var]}
        for neighbor in self.crossword.neighbors(var):
            if neighbor not in assignment:
        ...
        for x in l:
            vlist.append(x)
        return sorted(vlist, key = l.__getitem__,reverse= True)
        ...
        for v in self.crossword.variables:
            if v not in assignment:
                if len(self.domains[v]) < value:
                    ...
                    pass
        neighbors_count = {v: 0 for v in var}
        for v in var:
            for neighbor in self.crossword.neighbors(v):
                neighbors_count[v] += 1
        sorted(var,key = neighbors_count.__getitem__)
        return var[0]
    ...
    def backtrack(self, assignment):
        ...
        a = copy.deepcopy(self.order_domain_values(var,assignment))
        for value in self.order_domain_values(var,assignment):
            assign = copy.deepcopy(assignment)
            assign[var] = value
            if self.consistent(assignment=assign):
