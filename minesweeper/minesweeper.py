...
class MinesweeperAI():
    ...
    def add_knowledge(self, cell, count):
        ...
        for di in [-1,0,1]:
            for dj in [-1,0,1]:
                ni = i + di
                nj = j + dj
                if (0 <= ni < self.width) and (0 <= nj < self.height) and (ni, nj) not in self.safes and (ni, nj) not in self.mines and (ni, nj) != cell:
                    neighbors.add((ni,nj))
    ...
    def make_random_move(self):
        ...
        """
        all_cells = set((i, j) for i in range(self.width) for j in range(self.height))
